<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'" label="销售单号" prop="xiaoshoudanhao">
            <el-input v-model="ruleForm.xiaoshoudanhao" 
                placeholder="销售单号" readonly></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.xiaoshoudanhao" label="销售单号" prop="xiaoshoudanhao">
              <el-input v-model="ruleForm.xiaoshoudanhao" 
                placeholder="销售单号" readonly></el-input>
          </el-form-item>
        </div>
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="总价" prop="zongjia">
          <el-input v-model="ruleForm.zongjia" 
              placeholder="总价" clearable  :readonly="ro.zongjia"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="总价" prop="zongjia">
              <el-input v-model="ruleForm.zongjia" 
  ...
  methods: {
      ...
      this.$http({
		...
		if(json.shouji!=''&&json.shouji){
          ...
          this.$message.error(data.msg);
            ...
            this.leixingOptions = "退货,换货".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `shouhoufuwu/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `shouhoufuwu/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.shouhoufuwuCrossAddOrUpdateFlag = false;
                  this.parent.search();
